...
export function renderCoach(root){
  ...
  const form=root.querySelector('#wk');
  form.onsubmit=e=>{
    e.preventDefault();
    const v=Object.fromEntries(new FormData(form).entries());
    let msg="You logged again today. Nice work. Let us keep this week simple. Hit protein and a 300 kcal deficit on weekdays. Enjoy maintenance on Saturday.";
    const hunger=+v.hunger||0,sleep=+v.sleepHrs||0,stress=+v.stress||0;
    if(sleep<6) msg="Your weight is up today, likely water from salt or carbs. The trend is still on track.";
    if(hunger>7) msg="The last week was patchy. Let us reset with one small win today. Hit your protein target.";
    document.getElementById('suggest').innerText=msg;
