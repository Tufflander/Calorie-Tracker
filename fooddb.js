/**
 * fooddb.js — loads AU sample JSON, provides simple search & barcode index.
 * Quagga is loaded lazily only when scanning.
 */
let foods = [];
let barcodeIndex = new Map();
export async function ensureFoodDB(){
  if(foods.length) return;
  const res = await fetch('data/au_food_sample.json');
  foods = await res.json();
  barcodeIndex = new Map();
  foods.forEach(f=>{ if(f.ean) barcodeIndex.set(String(f.ean), f); });
}
export function searchFoods(q, exact=false){
  if(!q) return foods;
  if(exact){
    const byId = foods.find(f=>f.id===q);
    if(byId) return [byId];
    const byEAN = barcodeIndex.get(String(q));
    return byEAN ? [byEAN] : [];
  }
  const term = q.toLowerCase();
  return foods.filter(f=> (f.name+' '+(f.brand||'')).toLowerCase().includes(term) );
}
export function startBarcodeScan(cb){
  ...
  function start(){
    ...
    window.Quagga.init({
      inputStream: { name:'Live', type:'LiveStream', target: overlay.querySelector('#qg') },
